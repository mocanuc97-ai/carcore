...
test.describe('Dashboard and Core Flows', () => {
  ...
  test('should visit main sections and see core forms (layout + pages render)', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(800);
    await expect.soft(page.getByText(/Adaugă mașină/i)).toBeVisible({ timeout: 10000 });
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(800);
    await expect.soft(page.getByText(/Stoc Piese/i)).toBeVisible({ timeout: 10000 });

    await page.goto('/dashboard/appointments');
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(800);
    await expect.soft(page.getByText(/Programări/i)).toBeVisible({ timeout: 10000 });
  });
  ...
  test('should see add forms on clients and vehicles (data creation may be limited by RLS in test env)', async ({ page }) => {
    ...
    await page.waitForLoadState('networkidle');
    await page.waitForTimeout(800);
    await expect.soft(page.getByText(/Adaugă mașină/i)).toBeVisible({ timeout: 8000 });
  });
});
